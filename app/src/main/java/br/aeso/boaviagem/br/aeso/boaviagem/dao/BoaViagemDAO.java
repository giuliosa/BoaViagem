package br.aeso.boaviagem.br.aeso.boaviagem.dao;
...
public class BoaViagemDAO {
    ...
    public long inserirViagem(Viagem viagem){
        ...
        return getDatabase().insert(DatabaseHelper.Viagem.TABELA, null, values);
    }

    public boolean removerViagem(Long id){
        String whereClause = DatabaseHelper.Viagem._ID + " = ?";
        String[] whereArgs = new String[]{id.toString()};
        int removidos = getDatabase().delete(DatabaseHelper.Viagem.TABELA, whereClause,whereArgs);
        return removidos > 0;
    }

    public long editarViagem(Viagem viagem){
        ContentValues values = new ContentValues();
        values.put(DatabaseHelper.Viagem.DESTINO, viagem.getDestino());
        values.put(DatabaseHelper.Viagem.TIPO_VIAGEM, viagem.getTipoViagem());
        values.put(DatabaseHelper.Viagem.DATA_CHEGADA, viagem.getDataChegada().getTime());
    ...
    public List<Gasto> listarGastos(Viagem viagem){
        ...
        Cursor cursor = getDatabase().query(DatabaseHelper.Gasto.TABELA, DatabaseHelper.Gasto.COLUNAS,
    ...
    public Gasto buscarGastoPorId(Integer id){
        Cursor cursor = getDatabase().query(DatabaseHelper.Gasto.TABELA, DatabaseHelper.Gasto.COLUNAS,
                DatabaseHelper.Gasto._ID + " = ?", new String[]{id.toString()}, null,null,null);
        if(cursor.moveToNext()){
            Gasto gasto = criarGasto(cursor);
            cursor.close();
            return gasto;
    ...
    public long inserirGasto(Gasto gasto){
        ...
        values.put(DatabaseHelper.Gasto.LOCAL, gasto.getDescricao());
        values.put(DatabaseHelper.Gasto.VALOR, gasto.getValor());
        values.put(DatabaseHelper.Gasto.VIAGEM_ID, gasto.getViagemId());

        return getDatabase().insert(DatabaseHelper.Gasto.TABELA, null, values);
    }

    public boolean removerGasto(Long id){
        String whereClause = DatabaseHelper.Gasto._ID + " = ?";
        String[] whereArgs = new String[]{id.toString()};
        int removidos = getDatabase().delete(DatabaseHelper.Gasto.TABELA, whereClause, whereArgs);

        return removidos > 0;
    }

    public double calcularTotalGasto(Viagem viagem){
        Cursor cursor = getDatabase().rawQuery("SELECT SUM("+DatabaseHelper.Gasto.VALOR +") FROM "
